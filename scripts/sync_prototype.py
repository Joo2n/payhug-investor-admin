...
투자자 공유 정리 (원본 생성기에도 같은 정리가 있어 보통은 「원본에 이미 없음」으로 지나간다 — 여기는 이중 안전장치 · 탭 제목은 시연본만)
...
def investor_share(s):
    """투자자 공유 정리 — 원본 생성기(build_app.py 등)에 같은 정리가 있어 보통은 앵커 0건으로 지나가고, 남아 있으면 여기서 뺀다. 탭 제목은 시연본에서만 바꾼다.
