...
전체본은 `main` 에 올라간 63개 파일을 그대로 서비스한다. 한글 이름을 쓰는 PDF·TXT·XLSX 도 같은 주소에서 바로 열린다.
...
| `app.html` | 통합 프로토타입. 화면 14 · 상태 18 를 한 파일에서 조작. 메뉴 전환·엑셀 실제 내려받기·모달·검색·페이지네이션 동작. `#화면/상태` 해시 딥링크 |
