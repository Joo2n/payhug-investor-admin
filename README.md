...
- 2026-08-28 — 정산채권 양수 서명 대기 목록과 계약기록 재양도합의서 목록은 행을 눌러 고른다. 행에 마우스를 올리면 배경이 `primary-50/30` 으로 바뀌고 커서가 손가락이 되며 1.005배로 살짝 커진다(운영 어드민 `sales/page.tsx:130` · `inquiries/page.tsx:286` · `manage/page.tsx:291` 의 클릭 가능 행 값 + 미세 확대). 확대는 오른쪽 모서리를 축으로 삼아 표 폭·행 높이·이웃 행 위치를 밀지 않는다. 키보드 포커스에도 같은 확대와 함께 `primary-500` 2px 아웃라인이 붙는다. 서명이 끝난 행에는 신호를 주지 않는다. 선택 건수는 두 화면 다 목록 아래 왼쪽에 있고, 서명 버튼은 목록 카드 안 가운데 그대로다. 계약기록 표 맨 왼쪽에 순번 열 `No` 가 붙어 쪽이 넘어가도 이어지는 번호(1–8 · 9–16)를 보여준다 — 체크박스 열이 앞, 순번이 그다음이다. 범위 표기 `표시 a–b` 는 계약기록·가맹점 두 표에서 함께 내렸다. 숫자·산식·건수는 종전 그대로다.
- 2026-08-28 — 투자 자산 화면의 용어 박스 4줄(`금융일수` · `W금융일수` · `S입금부족율` · `Ty수익율`)이 통합본 `app.html` 과 정적 화면 5종(`invest-assets*.html`)에 같은 자리·같은 문구로 들어간다. 스토리보드 슬라이드 3·4·5·6 각주가 근거이며, 각주가 없는 슬라이드 7·8·9 계열(`invest-profit*`)과 증명서 화면에는 두지 않는다. `build_app.py` 가 문구를 `invest-assets.html` 에서 그대로 떠오므로 통합본과 정적 화면이 문자 단위로 같다. 숫자·산식·건수는 종전 그대로다.
- 2026-08-28 — 화면에서 스토리보드 근거가 없는 안내·부연을 걷어낸다. 메뉴 제목 아래 한 줄 설명, 표 아래 각주, 빈 상태 부연, 엑셀 서식 캡션이 전 화면에서 빠지고 `확인 대상` 꼬리 문장은 `inquiry.html` 부록(D-7·D-13·D-14·D-17)에만 남는다. 화면·상태 이동 도크를 걷어 실제 어드민에 없는 떠 있는 요소를 없앴고, 딥링크 `#화면/상태`는 Figma 캡처용으로 유지한다. 투자 수익 조회 영역은 스토리보드 슬라이드7 구성 그대로 `시작일 ~ 종료일 · 일주일 · 금월 · 검색 · 초기화` 한 줄에 그 아래 `일별 · 월별`을 두고, `어제` 프리셋은 슬라이드에 없어 내렸다. 페이지네이션은 쪽번호를 가운데, 총 건수를 같은 줄 왼쪽에 둔다(운영 어드민 `activity-logs/page.tsx:339` 가운데 정렬 + `LockAccountDeposits.tsx:435` 왼쪽 건수). 정산채권 양수 서명 버튼은 화면 하단 고정 바에서 목록 카드 안으로 들어와 목록 길이를 따라오고, 목록에는 `전체 선택`·`선택 해제`와 행 클릭 선택(키보드 Space·Enter 포함)이 붙는다. 계약기록에도 `선택 해제`가 다운로드 버튼 왼쪽에 붙는다. 쿠콘 화면은 안내 문단을 한 문단으로 잇고 조회 가능 내역 체크리스트를 같은 카드 안으로 옮긴다. 합계 행의 가중평균 표기는 괄호를 떼고 숫자 아래 줄로 내려 행 높이가 밀리지 않는다. 수익 산정 기준(예시) 카드는 그대로 두되 `build_app.py` · `fix12_static.py` 의 `SHOW_FORMULA` 한 값으로 켜고 끈다. 숫자·요율·건수는 종전 그대로다.
